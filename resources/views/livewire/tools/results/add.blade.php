        ...
        <div class="content-date">
            <form wire:submit.prevent="saveResults">
                <div>
                    ...
                    <input wire:model.debounce.10800000ms="number" id="number" type="number" name="number" @error('number') class="error" @enderror>
                    @error('number') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="result">النتيجة</label>
                    <input wire:model.debounce.10800000ms="result" id="result" type="text" name="result" @error('number') class="error" @enderror>
                    @error('result') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="manager_name">اسم المدير</label>
                    <input wire:model.debounce.10800000ms="manager_name" id="manager_name" type="text" name="manager_name" @error('manager_name') class="error" @enderror>
                ...
                <div>
                    <label>السنة</label>
                    <select wire:model.debounce.10800000ms="year_id">
                ...
                <button type="submit">إضافة النتيجة</button>
            </form>
        </div>
    </div>
