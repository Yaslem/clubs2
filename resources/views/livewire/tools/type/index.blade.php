    ...
    <section wire:loading.class="loading-status" class="b-index" id="container">
        @if($indexType == true)
            <div class="head-date">
                ...
                <span wire:click="addType()" id="addUserType" class="addUser">إضافة نوع جديد</span>
            </div>
            @if($Types->count() >= 1)
                <div id="index">
                    <div class="b-container-date">
                        <table id="table">
                            ...
                            @foreach($Types as $type)
                                <tr>
                                    <td>{{$type->name}}</td>
                                    <td>{{$type->reservations->count()}}</td>
                                    <td>{{$type->des}}</td>
                                    <td>
                                        <span wire:click="editType({{$type->id}})">
            ...
            @else
                <span style="text-align: center;display: block;  color: gray;opacity: 0.5;">لا توجد أنواع مضافة.</span>
            @endif
        @endif
        @if($addType == true)
            <livewire:tools.type.add>
        @endif
        @if($editType == true)
            <livewire:tools.type.edit>
        @endif
    </section>
