...
<div>
    <section wire:loading.class="loading-status" class="b-index" id="container">
        @if($indexTime == true)
            <div class="head-date">
                <span class="cancel" id="cancelTime" wire:click="indexTools()">رجوع</span>
                @if($Times->count() >= 1)
                @else
                    <span wire:click="addTime()" id="addUserTime" class="addUser">إضافة وقت جديد</span>
                @endif
            </div>
            @if($Times->count() >= 1)
                <div id="index">
                        ...
                        <table id="table">
                            ...
                            @foreach($Times as $Time)
                                <tr>
                                    ...
                                    <td>{{$Time->notes}}</td>
                                    <td>
                                        <span wire:click="editTime({{$Time->id}})">
            ...
            @else
                <span style="text-align: center;display: block;  color: gray;opacity: 0.5;">لا توجد أوقات مضافة.</span>
            @endif
        @endif
        @if($addTime == true)
            <livewire:tools.time.add>
        @endif
        @if($editTime == true)
            <livewire:tools.time.edit>
        @endif
    </section>
