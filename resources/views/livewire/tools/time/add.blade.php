        ...
        <div class="content-date">
            <form wire:submit.prevent="saveTime">
                <div>
                    ...
                    <input wire:model.debounce.10800000ms="start" id="name" type="time" name="start" @error('start') class="error" @enderror>
                    @error('start') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="name">إلى</label>
                    <input id="name" type="time" wire:model.debounce.10800000ms="end" name="end" @error('end') class="error" @enderror>
