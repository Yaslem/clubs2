...
<div>
    <section wire:loading.class="loading-status" class="b-index" id="container">
        @if($indexAward == true)
            <div class="head-date">
                <span class="cancel" id="cancelAwardTool" wire:click="indexTools()">رجوع</span>
                <span wire:click="addAward()" id="addUserAwardTool" class="addUser">إضافة جائزة جديدة</span>
            </div>
            @if($awards->count() >= 1)
                <div id="index">
                        ...
                        <table id="table">
                            ...
                            @foreach($awards as $award)
                                <tr>
                                    ...
                                    <td>{{$award->des}}</td>
                                    <td>
                                        <span wire:click="editAward({{$award->id}})">
            ...
            @else
                <span style="text-align: center;display: block;  color: gray;opacity: 0.5;">لا توجد جوائز مضافة.</span>
            @endif
        @endif
        @if($addAward == true)
            <livewire:tools.award.add>
        @endif
        @if($editAward == true)
            <livewire:tools.award.edit>
        @endif
    </section>
