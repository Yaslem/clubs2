<div>
    <section wire:loading.class="loading-status" class="b-index" id="container">
        @if($indexCollege == true)
            <div class="head-date">
                <span class="cancel" id="cancelCollege" wire:click="indexTools()">رجوع</span>
                <span wire:click="addCollege()" id="addUserCollege" class="addUser">إضافة كلية جديدة</span>
            </div>
            @if($colleges->count() >= 1)
                <div id="index">
                        ...
                        <table id="table">
                            ...
                            @foreach($colleges as $college)
                                <tr>
                                    ...
                                    <td>
                                        <span wire:click="editCollege({{$college->id}})">
                                        ...
                                        <span onclick="swal({
                                        title: 'هل أنت متأكد؟',
                                        text: 'هل أنت متأكد من أنك تريد حذف الكلية  ' + '[ ' + '{{$college->name}}' + ' ]',
                                        icon: 'warning',
                                        buttons: {
                                        cancel: {
                                        text: 'إلغاء',
                                        ...
                                        if (willDelete) {
                                        @this.willDeleteCollege({{$college->id}});
                                        }
                                        });">
            ...
            @else
                <span style="text-align: center;display: block;  color: gray;opacity: 0.5;">لا توجد كليات مضافة.</span>
            @endif
        @endif
        @if($addCollege == true)
            <livewire:tools.college.add>
        @endif
        @if($editCollege == true)
            <livewire:tools.college.edit>
        @endif
    </section>
