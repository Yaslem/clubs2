<div>
    <div wire:loading.class="loading-status">
        <span class="cancel" wire:click="indexDate()">رجوع</span>
        <div class="content-date">
            <form wire:submit.prevent="saveDate">
                <div>
                    <label for="name">من</label>
                    <input wire:model.debounce.10800000ms="start" id="name" type="date" name="start" @error('start') class="error" @enderror>
                    @error('start') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="name">إلى</label>
                    <input id="name" type="date" wire:model.debounce.10800000ms="end" name="end" @error('end') class="error" @enderror>
                    @error('end') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="notes">ملاحظات</label>
                    <textarea id="notes" wire:model.debounce.10800000ms="notes" name="notes"></textarea>
                </div>
                <button type="submit">إضافة التاريخ</button>
            </form>
        </div>
    </div>
