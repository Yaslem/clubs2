        ...
        <div class="content-date " id="content-date">
            <form wire:submit.prevent="saveDate()">
                <div>
                    ...
                    <input wire:model.debounce.10800000ms="start" id="name" type="date" name="start" value="{{$start}}" @error('start') class="error" @enderror>
                    @error('start') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="name">إلى</label>
                    <input id="name" type="date" wire:model.debounce.10800000ms="end" name="end" value="{{$end}}" @error('end') class="error" @enderror>
