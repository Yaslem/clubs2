    ...
    <section wire:loading.class="loading-status" class="b-index" id="container">
        @if($indexDate == true)
            <div class="head-date">
                ...
                @if($dates->count() >= 1)
                @else
                    <span wire:click="addDate()" id="addUserDate" class="addUser">إضافة تاريخ جديد</span>
                @endif
            </div>
            @if($dates->count() >= 1)
                <div id="index">
                        ...
                        <table id="table">
                            ...
                            @foreach($dates as $date)
                                <tr>
                                    ...
                                    <td>{{$date->notes}}</td>
                                    <td>
                                        <span wire:click="editDate({{$date->id}})">
            ...
            @else
                <span style="text-align: center;display: block;  color: gray;opacity: 0.5;">لا توجد تواريخ مضافة.</span>
            @endif
        @endif
        @if($addDate == true)
            <livewire:tools.date.add>
        @endif
        @if($editDate == true)
            <livewire:tools.date.edit>
        @endif
    </section>
