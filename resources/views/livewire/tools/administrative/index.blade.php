<div>
    <section wire:loading.class="loading-status" class="b-index" id="container">
        @if($indexAdministrative == true)
            <div class="head-date">
                <span class="cancel" id="cancelAdministrative" wire:click="indexTools()">رجوع</span>
                <span wire:click="addAdministrative()" id="addUserAdministrative" class="addUser">إضافة وظيفة جديد</span>
            </div>
            @if($administratives->count() >= 1)
                <div id="index">
                        ...
                        <table id="table">
                            ...
                            @foreach($administratives as $administrative)
                                <tr>
                                    ...
                                    <td>{{$administrative->users->count()}}</td>
                                    <td>
                                        <span wire:click="editAdministrative({{$administrative->id}})">
                                        ...
                                        <span onclick="swal({
                                        title: 'هل أنت متأكد؟',
                                        text: 'هل أنت متأكد من أنك تريد حذف المستوى  ' + '[ ' + '{{$administrative->name}}' + ' ]',
                                        icon: 'warning',
                                        buttons: {
                                        cancel: {
                                        text: 'إلغاء',
                                        ...
                                        if (willDelete) {
                                        @this.willDeleteAdministrative({{$administrative->id}});
                                        }
                                        });">
            ...
            @else
                <span style="text-align: center;display: block;  color: gray;opacity: 0.5;">لا توجد وظائف مضافة.</span>
            @endif
        @endif
        @if($addAdministrative == true)
            <livewire:tools.administrative.add>
        @endif
        @if($editAdministrative == true)
            <livewire:tools.administrative.edit>
        @endif
    </section>
