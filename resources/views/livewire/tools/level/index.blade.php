<div>
    <section wire:loading.class="loading-status" class="b-index" id="container">
        @if($indexLevel == true)
            <div class="head-date">
                <span class="cancel" id="cancelLevel" wire:click="indexTools()">رجوع</span>
                <span wire:click="addLevel()" id="addUserLevel" class="addUser">إضافة مستوى جديد</span>
            </div>
            @if($levels->count() >= 1)
                <div id="index">
                        ...
                        <table id="table">
                            ...
                            @foreach($levels as $level)
                                <tr>
                                    ...
                                    <td>
                                        <span wire:click="editLevel({{$level->id}})">
                                        ...
                                        <span onclick="swal({
                                        title: 'هل أنت متأكد؟',
                                        text: 'هل أنت متأكد من أنك تريد حذف المستوى  ' + '[ ' + '{{$level->name}}' + ' ]',
                                        icon: 'warning',
                                        buttons: {
                                        cancel: {
                                        text: 'إلغاء',
            ...
            @else
                <span style="text-align: center;display: block;  color: gray;opacity: 0.5;">لا توجد مستويات مضافة.</span>
            @endif
        @endif
        @if($addLevel == true)
            <livewire:tools.level.add>
        @endif
        @if($editLevel == true)
            <livewire:tools.level.edit>
        @endif
    </section>
