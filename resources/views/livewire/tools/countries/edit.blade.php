<div>
    <div wire:loading.class="loading-status">
        <span class="cancel" wire:click="indexCountry()">رجوع</span>
        <div class="content-date " id="content-date">
            <form wire:submit.prevent="saveCountry()">
                <div>
                    <label for="name">الاسم</label>
                    <input wire:model.debounce.10800000ms="name" id="name" type="text" name="start" value="{{$name}}" @error('name') class="error" @enderror>
                    @error('name') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="code">الكود</label>
                    <input wire:model.debounce.10800000ms="code" id="code" type="text" name="code" value="{{$code}}" @error('code') class="error" @enderror>
                    @error('code') <span class="error">{{ $message }}</span> @enderror
                </div>
                <button type="submit">تعديل الجنسية</button>
            </form>
        </div>
    </div>
