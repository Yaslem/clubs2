<div>
    <div wire:loading.class="loading-status" id="index">
        <div class="count-activity-container">
            <span style="margin-bottom: 20px" class="cancel" wire:click="indexUsers()">رجوع</span>
            @if($usersTrashed->count() >= 1)
            ...
            @can('view', App\Models\User::class)
                <div class="b-container">
                    <table id="table">
                        ...
                        <tbody>
                        @foreach($usersTrashed as $userTrashed)
                            <tr>
                                <td>{{$userTrashed->name}}</td>
                                <td>
                                    <img style="
                                width: 30px;
                                height: 30px;
                                ...
                                align-items: center;
                            " src="{{asset('uploads/files/'.$userTrashed->avatar)}}">
                                </td>
                                <td>{{$userTrashed->username}}</td>
                                <td>{{$userTrashed->number_of_whatsapp}}</td>
                                <td>@if($userTrashed->club_id) {{$userTrashed->ClubStatus->name}} @else --@endif</td>
                                <td>{{$userTrashed->role}}</td>
                                <td>{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $userTrashed->deleted_at)->diffForHumans()}}</td>
                                <td>
                                    @can('restore' , App\Models\User::class)
                                        <span wire:click="restoreUser({{$userTrashed->id}})">
                                    ...
                                    @endcan
{{--                                        <span title="ليست لديك هذه الصلاحية." style="cursor: not-allowed">--}}
{{--                                            <svg title="ليست لديك هذه الصلاحية." width="24px" height="24px" viewBox="-51.2 -51.2 614.40 614.40" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"><rect x="-51.2" y="-51.2" width="614.40" height="614.40" rx="307.2" fill="#f1f6f5" strokewidth="0"></rect></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>redo</title> <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="work-case" fill="#3e3c3c" transform="translate(64.000000, 80.915055)"> <path d="M281.751611,1.42108547e-14 L392.836556,111.084945 L281.751611,222.169889 L251.581722,192 L311.109,132.459 L138.64576,132.459878 C85.7290667,132.459878 42.6666667,175.522278 42.6666667,228.438971 C42.6666667,281.355665 85.7290667,324.418278 138.64576,324.418278 L213.541547,324.418278 L213.541547,367.084945 L138.64576,367.084945 C62.18752,367.084945 -1.42108547e-14,304.897425 -1.42108547e-14,228.439185 C-1.42108547e-14,151.980945 62.18752,89.7934247 138.64576,89.7934247 L311.193,89.793 L251.581722,30.1698893 L281.751611,1.42108547e-14 Z" id="Combined-Shape"> </path> </g> </g> </g></svg>--}}
{{--                                        </span>--}}
{{--                                    @endcan--}}
                                    @can('forceDelete' , App\Models\User::class)
                                        <span wire:click="forceDeleteUser({{$userTrashed->id}})">
        ...
        <div class="i-pagination">
            {{$usersTrashed->links('pagination.default')}}
        </div>
    </div>
