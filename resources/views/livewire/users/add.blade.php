        ...
        <div class="content-profile">
            <form wire:submit.prevent="store">
                <div>
                    ...
                    <input id="name" type="text" placeholder="اكتب الاسم كاملا" wire:model.debounce.10800000ms="name" name="name"  @error('name') class="error"  @enderror>
                    @error('name') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="username">الرقم الجامعي</label>
                    <input id="username" type="text" placeholder="أدخل الرقم الجامعي" wire:model.debounce.10800000ms="username" name="username" @error('username') class="error" @enderror>
                    @error('username') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="ID_Number">رقم الهوية/الإقامة</label>
                    <input id="ID_Number" type="text" placeholder="أدخل رقم الهوية/الإقامة" wire:model.debounce.10800000ms="ID_Number" name="ID_Number" @error('ID_Number') class="error" @enderror>
                ...
                <div>
                    ...
                    <input id="number_of_whatsapp" type="text" wire:model.debounce.10800000ms="number_of_whatsapp" placeholder="اكتب رقم الواتساب" name="number_of_whatsapp"  @error('number_of_whatsapp') class="error" @enderror>
                    @error('number_of_whatsapp') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="email">البريد الإلكتروني</label>
                    <input id="email" type="email" placeholder="أدخل البريد الإلكتروني" wire:model.debounce.10800000ms="email" name="email"  @error('email') class="error" @enderror>
                ...
                <div>
                    ...
                    <select id="club_id" wire:model.debounce.10800000ms="club_id" name="club_id">
                        ...
                        @if(Auth::user()->role === 'مدير الموقع' || Auth::user()->role === 'رئيس' || Auth::user()->role === 'منسق')
                            ...
                            @endforeach
                        @else
                            <option value="{{Auth::user()->ClubStatus->id}}">{{Auth::user()->ClubStatus->name}}</option>
                        @endif
                    </select>
