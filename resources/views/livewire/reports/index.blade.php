...
<div>
    <section class="b-index" id="container">
        @if($indexReports == true)
            <div wire:loading.class="loading-status" id="index">
                <div class="count-activity-container">
                    @can('createReport', App\Models\User::class)
                    <span wire:click="addReport()" class="addUser">إضافة تقرير جديد</span>
                    @endcan
                    <div class="count-activity">
                        <div>
                            <svg width="50px" height="50px" viewBox="-5.4 -5.4 28.80 28.80" xmlns="http://www.w3.org/2000/svg" mirror-in-rtl="true" fill="#000000" transform="rotate(270)"><g id="SVGRepo_bgCarrier" stroke-width="0"><rect x="-5.4" y="-5.4" width="28.80" height="28.80" rx="14.4" fill="#ecf9ff" strokewidth="0"></rect></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill="#425e70" d="M15 0H3a1 1 0 0 0-1 1v16a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V1a1 1 0 0 0-1-1zM6 14.983a1 1 0 1 1-2 0v-2a1 1 0 0 1 2 0v2zM4 5.5A2.5 2.5 0 0 1 6.5 3v2.5H9a2.5 2.5 0 0 1-5 0zm6 9.468a1 1 0 1 1-2 0v-5a1 1 0 0 1 2 0v5zM7.5 4.5V2A2.5 2.5 0 0 1 10 4.5H7.5zM14 15a1 1 0 0 1-2 0V4a1 1 0 0 1 2 0v11z"></path> </g></svg>                        </div>
                        <div>
                ...
                <div class="table-search">
                    ...
                    <div>
                        <label>البحث</label>
                        <input wire:model="titleSearch" type="text" placeholder="البحث تقرير باستخدام عنوان الفعالية" @if($titleIsNull == true) disabled style="opacity: 0.4;border: 0; cursor: not-allowed;" @endif>
                    </div>
                    @else
                ...
                <div class="b-container">
                    <table id="table">
                        ...
                        <tr>
                            ...
                            <th>التاريخ</th>
                            <th>حالة الفعالية</th>
                            @canany(['updateReport', 'deleteReport'], App\Models\User::class)
                            <th>الخيارات</th>
                            @endcan
                        </tr>
                        </thead>
                        ...
                        @if($reportsAll)
                            @foreach($reportsAll as $report)
                                <tr>
                                    <td>{{Str::limit($report->reservation->title, 20)}}</td>
                                    <td>{{$report->club->name}}</td>
                                    <td>{{$report->number_of_attendees}}</td>
                                    <td>{{$report->location->name}}</td>
                                    <td>{{$report->reservation->from}}</td>
                                    <td>{{$report->reservation->to}}</td>
                                    <td>{{$report->reservation->date}}</td>
                                    <td>{{$report->reservation->status}}</td>
                                    @canany(['updateReport', 'deleteReport'], App\Models\User::class)
                                    <td>
                                        @can('updateReport', App\Models\User::class)
                                        <span wire:click="editReport({{$report->id}})">
                                        ...
                                        @endcan
                                        @can('deleteReport', App\Models\User::class)
                                        <span onclick="areYouDelete('{{$report->id}}','{{$report->title}}')">
                                            <svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg"><path
                                                    opacity=".4"
                ...
                <div class="i-pagination">
                    {{$reportsAll->links('pagination.default')}}
                </div>
                @else
                    <span style="text-align: center;width: 100%;display: block;font-size: 30px;color: gray;margin-top: 30px;">لا توجد تقارير.</span>
                @endif
            </div>
        @endif
    ...
    <script>
        function areYouDelete(id, title) {
            swal({
                title: "هل أنت متأكد؟",
                text: "هل أنت متأكد من أنك تريد حذف التقرير  " + '[ ' + title + ' ]',
                icon: "warning",
                buttons: {
                    cancel: {
                        text: "إلغاء",
