        ...
        <div class="content-profile">
            <form wire:submit.prevent="store" enctype="multipart/form-data">
                <div>
                    ...
                    <input type="text" wire:model.debounce.10800000ms="title" name="title" value="{{$title}}">
                    @error('title') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="body">المحتوى</label>
                    <textarea id="body" placeholder="اكتب هنا محتوى المنشور" wire:model.debounce.10800000ms="body" name="body">{{$body}}</textarea>
                ...
                <div>
                    ...
                    <input id="image" type="file" wire:model.debounce.10800000ms="image" name="image" value="{{$image}}"  style="display: none" accept="image*">
                    @error('image') <span class="error">{{ $message }}</span> @enderror
                </div>
                @if ($image)
                    <div>
                        <img style="border-radius: 6px;border: 1px solid #ccc;" src="{{asset('uploads/files/'.$image)}}">
                    </div>
                @endif
