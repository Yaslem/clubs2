...
@section('style')
    <style>
        ...
        .content-profile .certificateImg{
            grid-column: span 4;
        ...
        .content-profile .certificateImg img{
            width: 100%;
            height: 100%;
        }
        .select-btn .btn-text{
...
<div>
    <section class="b-index" wire:loading.class="loading-status">
        @if($indexCertificate == true)
            @can('createCertificate', App\Models\User::class)
            <div class="count-activity-container">
                <span wire:click="addCertificateIndex" class="addUser">إضافة شهادة جديدة </span>
            </div>
            @endcan
            @if($reservations->count() > 0)
                <div class="b-index-container-index">
                            ...
                            <div class="b-index-content">
                                ...
                                <div>
                                    <ul>
                                        ...
                                        <li>
                                            <span>العدد: {{$reservation->certificates->certificate->count()}}</span>
                                        </li>
                                    </ul>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span style="width: 40%;display: block;background-color: #2f906f; cursor: pointer" wire:click="showCertificate({{$reservation->id}})">مشاهدة</span>
                                    @endcan
                                    @can('deleteCertificate' , App\Models\User::class)
                                    <span onclick="areYouDelete('{{$reservation->id}}', '{{$reservation->title}}')" style="width: 40%;background-color: #bc5656; cursor: pointer">حذف</span>
                                    @endcan
                                </div>
                            </div>
                        </div>
        ...
        @endif
        @if($addCertificateIndex == true)
            <livewire:certificates.add-certificate>
        @endif
    </section>
    ...
    <script>
        function areYouDelete(id, title) {
            swal({
                title: "هل أنت متأكد؟",
                text: "هل أنت متأكد من أنك تريد حذف الشهادة  " + '[ ' + title + ' ]',
                icon: "warning",
                buttons: {
                    cancel: {
                        text: "إلغاء",
