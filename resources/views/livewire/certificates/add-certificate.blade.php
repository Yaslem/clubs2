<div>
    <div wire:loading.class="loading-status" class="b-container">
        <span class="cancel" wire:click="index()">رجوع</span>
        <div class="content-profile">
            <form wire:submit.prevent="saveActivity()">
                <div style="grid-column: span 4;">
                    <div style="width: 100%;">
                        <label>عنوان الفعالية</label>
                        <select wire:model.debounce.10800000ms="activityId">
                            <option>اختر عنوان الفعالية</option>
                            @if($activities)
                                @foreach($activities as $activity)
                                    <option value="{{$activity->id}}">{{$activity->title}} = ({{$activity->club->name}})</option>
                                @endforeach
                            @else
                            @endif
                        </select>
                        @error('activityId') <span>{{$message}}</span> @enderror
                    </div>
                </div>
                <button>رفع الشهادة</button>
            </form>
        </div>
    </div>
