...
<div>
    <section class="b-index" wire:loading.class="loading-status">
        @if($indexAward == true)
            ...
            <div class="count-activity-container">
                <span wire:click="addAwardIndex" class="addUser">إضافة جائزة جديدة </span>
            </div>
            @endcan
                        ...
                        <div class="b-index-container">
                            <div class="b-index-content">
                                ...
                                <div>
                                    <ul>
                                        <li>
                                            ...
                                            <span>@if($reservation->club){{$reservation->club->name}}@else @endif</span>
                                        </li>
                                        <li>
                                            <span>العدد: {{$reservation->award->award->count()}}</span>
                                        </li>
                                    </ul>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <span style="width: 40%;display: block;background-color: #2f906f; cursor: pointer" wire:click="showAward({{$reservation->id}})">مشاهدة</span>
                                    @endcan
        ...
        @endif
        @if($showAward == true)
            <livewire:awards.show>
        @endif
        @if($addAwardIndex == true)
            <livewire:awards.add-award>
        @endif
    </section>
    ...
    <script>
        function areYouDelete(id, title) {
            swal({
                title: "هل أنت متأكد؟",
                text: "هل أنت متأكد من أنك تريد حذف الجائزة  " + '[ ' + title + ' ]',
                icon: "warning",
                buttons: {
                    cancel: {
                        text: "إلغاء",
